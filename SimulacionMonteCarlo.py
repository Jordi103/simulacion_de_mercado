...
class MertonJumpDiffusion(GBM):
    ...
    def ajustar_parametros(self, ts, M):
        ...
        def neg_log_likelihood(Theta):
            ...
            for n in range(1, new_params['N']):
                ...
                for k in range(k_MAX+1):
                    ...
                    scale = np.sqrt(ts[n-1]**2*new_params['Deltat']*sigma**2+k**2*sigma_J**2)
        ...
        def jac(Theta):
            ...
            for n in range(1, new_params['N']):
                ...
                for k in range(k_MAX+1):
                    ...
                    Sigma = (ts[n-1]*sigma)**2*new_params['Deltat'] + (k*sigma_J)**2
        ...
        opt_res = scipy.optimize.minimize(neg_log_likelihood, 0.5*np.ones(5), method='Newton-CG',
                                          callback=callback, jac=jac,
                                          options={'xatol': 1e-4, 'disp': False, 'verbose':1}, )
